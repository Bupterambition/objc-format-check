...
# "#pragma KEEP Formatter Exempt" or "// MARK: KEEP Formatter Exempt" means don't format this file.
# Read the first line and trim it.
line="$(head -1 "$1" | xargs)" 
if [ "$line" == "#pragma KEEP Formatter Exempt" -o "$line" == "// MARK: KEEP Formatter Exempt" ]; then
  cat "$1" && exit 0
fi
